	...
	for {
		...
		if err != nil {
			if err != dnsmessage.ErrSectionDone {
				log.Println(errors.New("failed to parse answer section for domain: ", ah.Name.String()))
			}
			break
		}
		...
		case dnsmessage.TypeA:
			...
			if err != nil {
				log.Println(errors.New("failed to parse A record for domain: ", ah.Name))
				break L
			}
		...
		default:
			if err := parser.SkipAnswer(); err != nil {
				log.Println(errors.New("failed to skip answer"))
				break L
			}
			continue
		}
