...
func (p *ports) SetProxyOnMacOS() string {
	...
	case p.status == true:
		for _, service := range NetworkServices {
			...
			commands += "/usr/sbin/networksetup" + "-setwebproxy" + service + "127.0.0.1 " + string(p.portHttpWithPac) + "\n"
			commands += "/usr/sbin/networksetup" + "-setsocksfirewallproxy" + service + "127.0.0.1 " + string(p.portSocks5) + "\n"
		}
	case p.status == false:
		for _, service := range NetworkServices {
