...
func GetRemoteGFWListUpdateTime(c *http.Client) (gfwlist GFWList, err error) {
	...
	resp, err := httpClient.HttpGetUsingSpecificClient(c, "https://api.github.ink/repos/v2rayA/dist-v2ray-rules-dat/tags")
	if err != nil {
...
func UpdateLocalGFWList() (localGFWListVersionAfterUpdate string, err error) {
	...
	u := fmt.Sprintf(`https://github.ink/v2rayA/dist-v2ray-rules-dat/raw/%v/geosite.dat`, gfwlist.Tag)
	if err = asset.Download(u, pathSiteDat+".new"); err != nil {
		log.Warn("UpdateLocalGFWList: %v", err)
		return
	}
	u2 := fmt.Sprintf(`https://github.ink/v2rayA/dist-v2ray-rules-dat/raw/%v/geosite.dat.sha256sum`, gfwlist.Tag)
