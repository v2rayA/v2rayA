...
func init() {
	infra.RegisterDialer("simpleobfs", NewSimpleObfsDialer)
}

// NewSimpleObfsDialer returns a simple-obfs proxy dialer.
func NewSimpleObfsDialer(s string, d infra.Dialer) (infra.Dialer, error) {
...
func NewSimpleObfs(s string, d infra.Dialer) (*SimpleObfs, error) {
	u, err := url.Parse(s)
	if err != nil {
