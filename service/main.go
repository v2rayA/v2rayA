...
func main() {
	...
	if err != nil {
		...
		fmt.Println("1. 本程序会修改v2ray配置文件")
		fmt.Println("2. 本程序生成的v2ray配置文件将占用10800(socks)、10801(http)、10802(http)端口，如果端口已被占用将会出错，如需修改端口请按照v2ray文档修改models/template.go的templateJson变量")
		fmt.Println("3. 如果程序未以root权限运行，可能会运行失败")
		fmt.Println("4. 如果在非docker环境下出现bug，建议删除运行目录下的.tr文件重新尝试")
