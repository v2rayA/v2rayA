...
func applySubscriptionSelection(index int, old *configure.SubscriptionRaw, servers []serverObj.ServerObj, info string, probe subscriptionProber, restart bool) error {
	...
	updated.Add(configure.NodeRef{TYPE: configure.SubscriptionServerType, Sub: index, ID: best + 1, Outbound: "proxy"})
	// Preserve current-main group balancing for subscriptions that own a group.
	if !old.PreferFirst && (old.AutoSelect || configure.GetConnectedServersByOutbound("proxy").Len() > 1) {
		for i := range servers {
			if i != best {
				updated.Add(configure.NodeRef{TYPE: configure.SubscriptionServerType, Sub: index, ID: i + 1, Outbound: "proxy"})
			}
		}
		unchanged = false
	}
	...
	next.Info = info
	wasRunning := v2ray.ProcessManager.Running() || (restart && (configure.GetRunning() || configure.GetLastKernelExitStatus() == configure.LastKernelExitCrashed))
