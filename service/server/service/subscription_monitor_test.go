...
func TestMonitorRecoveryCanBeCancelledWithoutBlockingSettings(t *testing.T) {
	...
	if target == nil {
		t.Fatalf("missing target: running=%v refs=%+v sub=%+v", configure.GetRunning(), configure.GetConnectedServersByOutbound("proxy").Get(), configure.GetSubscription(0))
	}
...
func TestFirstEntryPolicyNeverFallsBack(t *testing.T) {
	...
	if err := Connect(&configure.NodeRef{TYPE: configure.SubscriptionServerType, Sub: 0, ID: 2, Outbound: "proxy"}); err == nil {
