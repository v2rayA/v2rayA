...
func activeMonitorTarget() *monitorTarget {
	if !configure.GetRunning() && configure.GetLastKernelExitStatus() != configure.LastKernelExitCrashed {
		return nil
	}
	ws := configure.GetConnectedServersByOutbound("proxy").Get()
	if len(ws) == 0 || ws[0].TYPE != configure.SubscriptionServerType {
		return nil
	}
	w := ws[0]
	if !subscriptionOwnsProxy(w.Sub) {
		return nil
	}
...
func sameMonitorTarget(a, b *monitorTarget) bool {
	return a != nil && b != nil && a.key == b.key && a.template == b.template && reflect.DeepEqual(a.subscription, b.subscription) && reflect.DeepEqual(a.references, b.references)
}
