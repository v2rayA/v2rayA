...
def main():
    ...
    try:
        assert api('version')['coreVersionValid'], api('version')
        ...
        ssh("kill -9 $(pgrep -f '^/usr/bin/v2raya_core run --config=/etc/v2raya/config.json$')")
        ...
        record('final memory and cleanup',details=ssh("free; pgrep -f '^/usr/bin/v2raya_core run '; find /tmp -maxdepth 1 -name 'v2raya-subscription-*'"))
    finally:
        sub.release.set()
