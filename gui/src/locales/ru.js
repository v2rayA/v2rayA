export default {
  ...
  subscription: {
    autoUpdate: "Автоматически обновлять подписку",
    autoUpdateHelp:
      "Обновляет только список серверов. Автоматическое наполнение настраивается в группе прокси.",
    updateIntervalMinutes: "Плановый интервал (минуты)",
    regularHelp: "Обязательное поле. 0 отключает плановые обновления.",
    failureIntervalMinutes: "При сбоях (минуты)",
    failureHelp: "Обязательно, минимум 1 минута. Обновляет, пока все серверы недоступны; прекращает попытки, когда заработает хотя бы один.",
  ...
  outbound: {
    ...
    autoAddHelp: "Наполняет группу доступными серверами из всей вкладки «Прокси», включая все подписки.",
    autoAddDetails: "Проверяет после обновления списка и с заданным интервалом. Убирает недоступные серверы из группы. Пустая группа блокирует свой трафик.",
