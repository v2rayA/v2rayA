export default {
  ...
  setting: {
    ...
    options: {
      ...
      leastPing: "최저 지연 우선",
      tunBackendTun: "TUN (기본값, /proc 프로세스 조회)",
    ...
    messages: {
      ...
      transparentType:
        "★tproxy: UDP를 지원하지만 Docker는 지원하지 않습니다. ★redirect: Docker에 적합하지만 UDP를 지원하지 않으며 DNS 오염 방지를 위해 로컬 포트 53을 사용합니다. ★tun (TinyTun): tinytun 바이너리가 필요한 크로스 플랫폼 TUN 기반 프록시입니다. Linux에서는 데이터 경로를 TinyTun 자체가 처리하므로 생성된 YAML이 TinyTun 버전과 일치해야 합니다.",
      ...
      tunAutoRoute:
        "활성화하면 TinyTun이 시스템 라우트를 자동으로 구성합니다. 비활성화하면 라우팅을 직접 구성할 사용자 지정 시작/종료 스크립트를 제공해야 합니다.",
      tunProcessBackend:
        "Linux에서 TinyTun의 프로세스 제외 방식을 선택합니다. TUN 모드는 폭넓은 호환성을 위해 /proc 스캔을 사용하고, eBPF 모드는 더 정확한 프로세스 단위 제외를 위해 cgroupv2 훅을 사용하며 /usr/lib/tinytun/에 tinytun-ebpf.o가 필요합니다.",
      tunExcludeProcesses:
        "TinyTun을 우회할 추가 프로세스 이름을 설정합니다. 한 줄에 하나씩 입력하는 것이 좋습니다. 예: chrome.exe 또는 firefox.",
  ...
  tinytun: {
    routeScript: {
      ...
      setupScript: "설정 스크립트 (TinyTun 시작 후 실행)",
      setupScriptPlaceholder: "# TinyTun 시작 시 라우트를 구성하는 스크립트\n# 예: ip route add default dev tun0",
      teardownScript: "해제 스크립트 (TinyTun 중지 전 실행)",
      teardownScriptPlaceholder: "# TinyTun 중지 시 라우트를 제거하는 스크립트\n# 예: ip route del default dev tun0",
    },
    processExclude: {
