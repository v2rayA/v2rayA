...
Узлы используются через группы прокси. Группа `proxy` существует всегда; остальные создаются, настраиваются и удаляются через кнопку групп в верхней панели — это единственное место для управления группами. Узел добавляется в группу из своего меню, либо выделите несколько на странице «Прокси» и выберите группу в **Добавить в группу**. Группа с несколькими подключёнными участниками использует участника с наименьшей измеренной задержкой (**Авто (минимальная задержка)**); меню узла на панели позволяет закрепить одного участника, чтобы группа использовала только его, **Добавить узлы** меняет состав участников, а в настройках группы задаются адрес и интервал проверки.
